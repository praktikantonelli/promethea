...
- Rationale: The server should remain the authoritative consistency boundary for all clients, enabling a local database to be used safely on the server.
- Acceptance Criteria:
...
- More Information: The current potential candidate for a database is SQLite, as tracked in [[REQ-POC-001]]. If that POC fails, other options shall be explored.